...
function dumpDex(moduleFunctionName, processName) {
    if (moduleFunctionName !== "") {
        var hookFunction;
        if (getAndroidVersion() > 4) { // android 5 and later version
            hookFunction = Module.findExportByName("libart.so", moduleFunctionName);
        } else { // android 4
            hookFunction = Module.findExportByName("libdvm.so", moduleFunctionName);  // check libdvm.so first
            if (hookFunction == null) {
                hookFunction = Module.findExportByName("libart.so", moduleFunctionName); //// if not load libdvm.so, check libart.so
            }
        }
...
//start dump dex file
var moduleFunctionName = getFunctionName();
console.log('moduleFunctionName: ', moduleFunctionName);

var processName = getProcessName();
console.log('processName: ', processName);

if (moduleFunctionName !== "" && processName !== "") {
    dumpDex(moduleFunctionName, processName);
}
